...
Focusd::Focusd (int in_argc, char **in_argv):rts2core::Device (in_argc, in_argv, DEVICE_TYPE_FOCUS, "F0")
{
	...
	extTempDevice = nullptr;
	extTempVariable = nullptr;

	linearOffset = nullptr;
	slope = nullptr;
	intercept = nullptr;
	temperatureNightOnly = nullptr;
...
{
	if (extTempDevice != nullptr && in_addr->isAddress (extTempDevice))
		return 1;
...
int Focusd::idle ()
{
	if (extTempDevice != nullptr && extTempVariable != nullptr)
	{
	...
	else if (linearOffset != nullptr && slope != nullptr && intercept != nullptr && temperature != nullptr && temperatureNightOnly != nullptr && linearOffset->getValueBool () == true && !std::isnan (slope->getValueFloat ()) && !std::isnan (intercept->getValueFloat ()) && !std::isnan (temperature->getValueDouble ()))
	{
