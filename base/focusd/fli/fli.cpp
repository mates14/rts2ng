...
// base note: like Andor/gxccd, the FLI SDK is NOT deferred - a real,
// already-built libfli lives at /home/mates/src/fliusb/libfli (the
// modern variant, using FLI's own "fliusb" kernel-module driver for USB
// I/O rather than raw usbfs - see STATUS.md's "FLI" section for the
// story of finding this after an earlier attempt against an older
// libfli-1.32 snapshot that needed patching and was missing
// FLIGetSerialString()). Ported completely faithfully against this
// version - no API-gap workarounds needed. Gated behind
// BASE_FLI_SDK_DIR (see top-level CMakeLists.txt), empty/skipped by
// default, same opt-in reasoning as Andor/gxccd (classic tree:
// --with-fli=<path>).
...
Fli::Fli (int argc, char **argv):Focusd (argc, argv)
{
	...
	addOption ('N', nullptr, 1, "focuser serial number (string, device name)");
...
int Fli::initHardware ()
{
	...
	if (fliDebug)
		FLISetDebugLevel (nullptr, FLIDEBUG_ALL);
	...
	if (ret)
	{
		logStream (MESSAGE_ERROR) << "cannot open device " << (name == nullptr ? names[0] : name) << ":" << strerror (errno) << sendLog;
		FLIFreeList (names);
		return -1;
	}
