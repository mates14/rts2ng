...
// base note: dropped the classic tree's #include <rts2-config.h> - nothing
// in this header uses an RTS2_HAVE_*/RTS2_* macro (the .cpp needs
// RTS2_CENTRALD_PORT and includes base-config.h itself).
// Also dropped the #ifndef HOST_NAME_MAX fallback - <climits> always
// provides it on glibc.
#include <climits>
...
class Centrald:public Daemon
{
	...
	private:
		...
		void maskCentralState (rts2_status_t state_mask, rts2_status_t new_state, const char *description = nullptr, double start = NAN, double end = NAN, Connection *commandedConn = nullptr);
