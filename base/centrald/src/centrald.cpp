...
void ConnCentrald::updateStatusWait (rts2core::Connection * conn)
{
	...
	else
	{
		if (statusCommandRunning == 0)
			return;
		if (getMaster ()->commandOriginatorPending (this, nullptr))
			return;
	}
...
{
	// base note (2026-07-16): used to also persist every message to
	// fileLog here (centrald was the sole writer of one shared log file
	// for the whole system) - removed now that every daemon, including
	// centrald itself, persists its own messages locally via syslog (see
	// Daemon::sendMessage). This just rebroadcasts for live monitoring
	// (rts2-mon etc.) - no persistence responsibility left here.
	sendMessageAll (msg);
}
