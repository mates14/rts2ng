...
// base note: data.cpp (not yet ported) calls real Connection methods
// (paramNextSSizeT, paramNextInteger, ...), same situation as value.cpp -
// blocked on task 5's Connection port. This header compiles standalone
// (Connection is only forward-declared/passed-through as a pointer).
// Added explicit <map>/<cstring> includes the classic header relied on
// getting transitively from connection.h.
...
{
	public:
		DataAbstractShared () { data = nullptr; shm_id = -1; }
...
class DataSharedRead: public DataAbstractRead, public DataAbstractShared
{
	public:
		 ...
		 */
		DataSharedRead () { data = nullptr; segment = -1; activeSegment = nullptr; shm_id = -1; }
