...
// base note: this REPLACES the earlier minimal logging-only placeholder
// (see STATUS.md "CRITICAL FINDING: Block extends App directly") - Block
// inherits from App for its constructor, run loop and timers, not just
// logging, so App had to be ported for real ahead of schedule. Ported
// faithfully as-is (per the user's call - see STATUS.md); the disputed
// networked-vs-syslog logging behavior actually lives in Daemon's override
// of sendMessage (task 7), not here - App::sendMessage below just writes to
// stderr, which is correct baseline CLI-tool behavior either way.
...
class App:public Object
{
	...
	protected:
		...
		{
			return (app_argv != nullptr && app_argv[0] != nullptr) ? app_argv[0] : "--child--";
		}
