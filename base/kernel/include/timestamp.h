...
class Timestamp
{
	public:
		...
		Timestamp (double _ts) : ts (_ts) {}
		Timestamp (time_t _ts) : ts (_ts) {}
...
{
	public:
		TimeDiff () : time_1 (0), time_2 (0), print_milisec (true) {}
		...
		double getTimeDiff () const { return time_2 - time_1; }
...
{
	public:
		TimeJDDiff () : TimeJD (), time_diff (0) {}

		TimeJDDiff (double in_time_jd, time_t in_time_diff) : TimeJD (in_time_jd), time_diff (in_time_diff) {}
...
{
	public:
		Percentage (double in_per, double in_total) : per (in_per), total (in_total) {}
