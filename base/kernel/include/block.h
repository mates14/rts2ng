...
// base note: the classic header redefined the PROTO_* constants here
// (duplicating proto.h verbatim) - just includes proto.h instead. Also
// dropped the RTS2_HAVE_SYS_INOTIFY_H conditional around <sys/inotify.h> -
// always present on a modern Linux target.
...
class Block: public App
{
	...
	public:
		 ...
		 */
		template <typename T> void queueCommandForType (int deviceType, T &command, int *numdev = nullptr, bool clearQueue = false)
		{
			if (numdev)
				*numdev = 0;
		 ...
		 * @see getValue
		 */
		Value *getValueExpression (std::string expression, const char *defaultDevice = nullptr);
