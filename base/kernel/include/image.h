...
// base note: classic tree has this at include/rts2fits/image.h -
// flattened to kernel/include/image.h, same convention as fitsfile.h/
// channel.h. The optional ImageMagick/libjpeg preview path
// (getMagickImage/writeLabel/writeAsJPEG/writeAsBlob, classic
// `#if RTS2_HAVE_LIBJPEG`) is dropped entirely - base has no
// Magick++/libjpeg dependency anywhere and this is cosmetic (JPEG preview
// generation), not needed by execcli/scriptexec. The classic
// `#if !RTS2_HAVE_DECL_LN_GET_HELIOCENTRIC_TIME_DIFF` fallback
// implementation of ln_get_heliocentric_time_diff() is also dropped - the
// libnova installed on this system already provides it natively
...
class Image:public FitsFile
{
	public:
		...
		void setObserver ()
		{
			setRTS2Value ("OBSERVER", "RTS2 " BASE_VERSION, "observer");
		}
