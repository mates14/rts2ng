...
// base note: value.cpp cannot be added to the build yet - its method
// bodies call real Connection methods (paramNextInteger, sendMsg,
// sendValueRaw, ...), not just a forward-declared type. This header
// compiles standalone fine (Connection is only forward-declared below);
// the .cpp is ported and waiting, wired into the build once task 5
// (Connection layer) lands.
...
class SelVal
{
	public:
		 ...
		 */
		SelVal (std::string in_name, Rts2SelData *in_data = nullptr)
		{
			name = in_name;
			data = in_data;
		}
...
class ValueSelection:public ValueInteger
{
	public:
		 ...
		 */
		void addSelVal (const char *sel_name, Rts2SelData *data = nullptr) { addSelVal (std::string (sel_name), data); }

		void addSelVal (std::string sel_name, Rts2SelData *data = nullptr) { addSelVal (SelVal (sel_name, data)); }
