...
class Rts2Target
{
	...
	public:
		Rts2Target ()
		{
			...
			target_type = 0;
			target_name = NULL;
			// base note: classic tree never initialized epochId here -
			// a genuine uninitialized-member bug (getEpoch() could
			// return garbage before the first setEpoch() call). See
			// UPSTREAM_BUGS.md. Fixed by defaulting to -1, matching
			// the same convention used by rts2core::Expander::epochId.
			epochId = -1;
		}
