...
// base note: classic tree has this at include/connection/udp.h - flattened
// to kernel/include/connudp.h, matching this port's flat kernel/include/
// convention (no connection/ subdirectory anywhere else in base either).
 ...
 * the hostname to nullptr and use the receive function.
 *
...
class ConnUDP:public ConnNoSend
{
	public:
		 ...
		 *
		 * @param hostname   hostname of the remote server to connect to, if left nullptr, we bind to the local port and serve as a server on all interfaces
		 */
		ConnUDP (int _port, rts2core::Block * _master, const char* hostname = nullptr, size_t _maxSize = 500);
		virtual int init ();
