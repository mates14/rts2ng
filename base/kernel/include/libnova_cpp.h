 ...
 *
 * base note: ra/dec were raw owning pointers in the classic tree, with a
 * hand-written destructor and copy constructor but no copy-assignment
 * operator - meaning `a = b` used the compiler-generated shallow copy and
 * double-freed on destruction. Switched to unique_ptr and added an explicit
 * deep-copy assignment operator to actually fix that instead of just hiding it.
 *
 * @author Petr Kubanek <petr@kubanek.net>
 *
 * @ingroup LibnovaCPP
...
class LibnovaRaDec
{
	public:
		...
		{
			ra = std::make_unique<LibnovaRa> (in_libnova.ra->getRa ());
			dec = std::make_unique<LibnovaDec> (in_libnova.dec->getDec ());
		}

		LibnovaRaDec (struct ln_equ_posn *pos)
		{
			ra = std::make_unique<LibnovaRa> (pos->ra);
			dec = std::make_unique<LibnovaDec> (pos->dec);
		}

		LibnovaRaDec (double in_ra, double in_dec)
		{
			ra = std::make_unique<LibnovaRa> (in_ra);
			dec = std::make_unique<LibnovaDec> (in_dec);
		}

		LibnovaRaDec & operator= (const LibnovaRaDec &in_libnova)
		{
			if (this != &in_libnova)
			{
				ra = std::make_unique<LibnovaRa> (in_libnova.ra->getRa ());
		...
		void setRa (double in_ra) { ra = std::make_unique<LibnovaRa> (in_ra); }
		...
		void setDec (double in_dec) { dec = std::make_unique<LibnovaDec> (in_dec); }
		...
		LibnovaRa *getRaObj () { return ra.get (); }

		LibnovaDec *getDecObj () { return dec.get (); }
	...
	private:
		std::unique_ptr<LibnovaRa> ra;
		std::unique_ptr<LibnovaDec> dec;
};
...
class LibnovaHrz
{
	public:
		...
		{
			alt = std::make_unique<LibnovaDeg90> (in_libnova.alt->getDeg ());
			az = std::make_unique<LibnovaDeg360> (in_libnova.az->getDeg ());
		}

		LibnovaHrz (struct ln_hrz_posn *hrz)
		{
			alt = std::make_unique<LibnovaDeg90> (hrz->alt);
			az = std::make_unique<LibnovaDeg360> (hrz->az);
		}

		LibnovaHrz (double in_alt, double in_az)
		{
			alt = std::make_unique<LibnovaDeg90> (in_alt);
			az = std::make_unique<LibnovaDeg360> (in_az);
		}

		LibnovaHrz & operator= (const LibnovaHrz &in_libnova)
		{
			if (this != &in_libnova)
			{
				alt = std::make_unique<LibnovaDeg90> (in_libnova.alt->getDeg ());
	...
	private:
		std::unique_ptr<LibnovaDeg90> alt;
		std::unique_ptr<LibnovaDeg360> az;
};
