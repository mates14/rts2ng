...
// base note: the classic tree also #included "block.h" here, but nothing in
// this file touches Block - dropped as an unused/accidental dependency.
#include "timestamp.h"
...
std::ostream & operator << (std::ostream & _os, TimeDiff _td)
{
	...
	if (!std::isfinite (_td.time_1) || !std::isfinite (_td.time_2))
	{
...
{
	if (!std::isfinite (_per.per) || !std::isfinite (_per.total) || _per.total == 0)
	{
