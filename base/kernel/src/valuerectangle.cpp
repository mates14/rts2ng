...
{
	x.reset (newValue (rts2Type & RTS2_BASE_TYPE, getName () + ".X", getDescription () + " X"));
	y.reset (newValue (rts2Type & RTS2_BASE_TYPE, getName () + ".Y", getDescription () + " Y"));
	w.reset (newValue (rts2Type & RTS2_BASE_TYPE, getName () + ".WIDTH", getDescription () + " WIDTH"));
	h.reset (newValue (rts2Type & RTS2_BASE_TYPE, getName () + ".HEIGHT", getDescription () + " HEIGHT"));
}
...
{
	snprintf (buf, VALUE_BUF_LEN, "%s %s %s %s", x->getValue (), y->getValue (), w->getValue (), h->getValue ());
	return buf;
}
