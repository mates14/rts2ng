...
// base note: NOT yet wired into kernel/CMakeLists.txt - needs block.h
// (task 6) for real, since Connection calls Block methods (addPollFD,
// deviceReady, createOtherType, addAddress, statusInfo, ...) and stores
// CommandDeviceStatus (command.h, already ported). Ported ahead of time
// against the classic connection.h/command.h/block.h interfaces.
//
// Also: the classic tree currently has some active in-progress debug
// scratch in sendMsg() (commented-out clock_gettime/debugtime lines, and a
// temporary std::cout swapped in for the usual logStream call under
// DEBUG_EXTRA) - that's someone's uncommitted WIP on the classic tree, not
// settled behavior, so it's not carried over here; this port keeps the
// plain logStream-based error path.
...
int Connection::paramNextString (char **str, const char *enddelim)
{
	...
	else
	{
		...
		*str = command_buf_top;
		while (!isspace (*command_buf_top)
		  && *command_buf_top
		  && (enddelim == nullptr || strchr (enddelim, *command_buf_top) == nullptr))
			command_buf_top++;
	}
	if (*command_buf_top)
	{
