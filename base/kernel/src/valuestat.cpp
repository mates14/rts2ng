...
// base note: NOT yet wired into kernel/CMakeLists.txt - needs connection.h
// to exist for real (task 5). Ported ahead of time against the connection.h
// interface already read in full; add to the build once Connection lands
// and verify then.
...
{
	snprintf (buf, sizeof (buf), "%.20le %i %.20le %.20le %.20le %.20le", value, numMes, mode, min, max, stdev);
	return buf;
}
...
const char * ValueDoubleStat::getDisplayValue ()
{
	...
	switch (getValueDisplayType ())
	{
		case RTS2_DT_DEGREES:
			...
			strncpy (buf, os.str ().c_str (), sizeof (buf) - 1);
			buf[sizeof (buf) - 1] = '\0';
			break;
...
{
	snprintf (buf, sizeof (buf), "%.20le %i %.20le %.20le %.20le %.20le %.20le %.20le", value, numMes, mode, min, max, stdev, alpha, beta);
	return buf;
}

const char * ValueDoubleTimeserie::getDisplayValue ()
{
	snprintf (buf, sizeof (buf), "%f %i %f %f %f %f %f %f", getValueDouble (), numMes, mode, min, max, stdev, alpha, beta);
	return buf;
}
