...
int ConnEthernet::writeRead (const void* wbuf, int wlen, void *rbuf, int rlen, long int wtime)
{
	...
	while (recvfrom (sockE, ethInBuffer, ETH_FRAME_LEN, MSG_DONTWAIT, nullptr, nullptr) > 0) {}
	...
	while (true)
	{
		...
		ret = select (FD_SETSIZE, &read_set, nullptr, nullptr, &readTout);
		gettimeofday(&actTime, nullptr);
		if (ret < 0)
		{
		...
		packetLength = recvfrom (sockE, ethInBuffer, ETH_FRAME_LEN, MSG_DONTWAIT, nullptr, nullptr);
		if (packetLength == -1)
		{
