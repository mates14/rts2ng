...
{
	if (getValue () == nullptr || other_value->getValue () == nullptr)
...
{
	snprintf (buf, VALUE_BUF_LEN, "%.20e", value);
	return buf;
}
...
const char * ValueFloat::getDisplayValue ()
{
	...
	if ((absv > 10e-3 && absv < 10e+5) || absv == 0)
		snprintf (buf, VALUE_BUF_LEN, "%lg", value);
	else
		snprintf (buf, VALUE_BUF_LEN, "%.20le", value);
	return buf;
}
...
{
	snprintf (buf, VALUE_BUF_LEN, "%li", value);
	return buf;
}
