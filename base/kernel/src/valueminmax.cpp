...
{
	snprintf (buf, VALUE_BUF_LEN, "%.20le %.20le %.20le", getValueDouble (), getMin (), getMax ());
	return buf;
}

const char * ValueDoubleMinMax::getDisplayValue ()
{
	snprintf (buf, VALUE_BUF_LEN, "%g %g %g", getValueDouble (), getMin (), getMax ());
	return buf;
}
...
{
	snprintf (buf, VALUE_BUF_LEN, "%d %d %d", getValueInteger (), getMin (), getMax ());
	return buf;
}
