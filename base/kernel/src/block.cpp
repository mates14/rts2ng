...
// base note: NOT yet wired into kernel/CMakeLists.txt - needs the whole
// Connection/Command/DevClient set, all ported and waiting.
//
// Two deliberate deviations from the classic file:
//  - dropped "#include \"client.h\"" - grep found nothing in this file
//    actually uses ConnClient/ConnCentraldClient/CommandLogin/Client from
//    it, same as the timestamp.cpp/block.h and iniparser.h/value.h dead
//    includes found earlier in this port.
//  - dropped the file-scope `type_names[]` array - also grep-verified
//    unused anywhere in the tree, and it had a real bug anyway (a missing
//    comma between "UNKNOWN" and "EXEC" silently string-concatenates them
//    into "UNKNOWNEXEC" at index 15, so the array doesn't even line up
//    with the DEVICE_TYPE_* constants it's named after).
//  - Block::createOtherType() below returns a plain DevClient for every
//    device type instead of switching to the 15 concrete DevClient
//    subclasses - see the note at the top of devclient.h.
...
{
	#ifdef DEBUG_EXTRA
	std::cout << "addConnection (): name " << _conn->getName () << ", centraldNum " << _conn->getCentraldNum () << ", centraldID " << _conn->getCentraldId () << ", key " << _conn->getKey () << ", connState " << _conn->getConnState () << std::endl;
	#endif
...
int Block::idle ()
{
	...
	#ifdef DEBUG_BRUTAL
	Connection *conn;
	connections_t::iterator iter2;
	for (iter2 = connections.begin (); iter2 != connections.end ();)
	{
		conn = *iter2;
		std::cout << "idle connections review: name " << conn->getName () << ", centraldNum " << conn->getCentraldNum () << ", centraldID " << conn->getCentraldId () << ", key " << conn->getKey () << ", connState " << conn->getConnState () << std::endl;
		iter2++;
	}
	for (iter2 = centraldConns.begin (); iter2 != centraldConns.end ();)
	{
		conn = *iter2;
		std::cout << "idle centraldConns review: name " << conn->getName () << ", centraldNum " << conn->getCentraldNum () << ", centraldID " << conn->getCentraldId () << ", key " << conn->getKey () << ", connState " << conn->getConnState () << std::endl;
		iter2++;
	}
	#endif
...
void Block::pollSuccess ()
{
	...
	for (iter = connections.begin (); iter != connections.end ();)
	{
		...
		{
			#ifdef DEBUG_EXTRA
			std::cout << "Will delete connection: name " << conn->getName () << ", centraldNum " << conn->getCentraldNum () << ", centraldID " << conn->getCentraldId () << ", key " << conn->getKey () << ", connState " << conn->getConnState () << std::endl;
			#endif
	...
	for (iter = centraldConns.begin (); iter != centraldConns.end ();)
	{
		...
		{
			#ifdef DEBUG_EXTRA
			std::cout << "Will delete centrald connection: name " << conn->getName () << ", centraldNum " << conn->getCentraldNum () << ", centraldID " << conn->getCentraldId () << ", key " << conn->getKey () << ", connState " << conn->getConnState () << std::endl;
			#endif
			ret = deleteConnection (conn);
			// delete connection only when it really requested to be deleted..
			if (!ret)
...
void Block::oneRunLoop ()
{
	...
	addPollSocks ();
	if (ppoll (fds, npolls, &read_tout, nullptr) > 0)
		pollSuccess ();
	ret = idle ();
	if (ret == -1)
		endRunLoop ();
...
int Block::setMasterState (Connection *_conn, rts2_status_t new_state)
{
	...
	if (stateMasterConn != nullptr && _conn != stateMasterConn)
	{
...
void Block::addAddress (int p_host_num, int p_centrald_num, int p_centrald_id, const char *p_name, const char *p_host, int p_port, int p_device_type)
{
	...
	#ifdef DEBUG_EXTRA
	std::cout << "addAddress: findAddress result: " << an_addr << std::endl;
	#endif
	if (an_addr)
	{
		ret = an_addr->update (p_centrald_num, p_name, p_host, p_port, p_device_type);
		#ifdef DEBUG_EXTRA
		std::cout << "addAddress: findaddress nenasel, update hodil: " << ret << std::endl;
		#endif
		if (!ret)
		{
			#ifdef DEBUG_EXTRA
			std::cout << "addAddress: findaddress nenasel, update hodil nulu, tak se vola addAddress" << std::endl;
			#endif
			addAddress (an_addr);
			return;
		}
	...
	#ifdef DEBUG_EXTRA
	std::cout << "addAddress: ok, takze melo dojit k vytvoreni new NetworkAddress a jeho push_bash do blockAdress... getName: " << an_addr->getName () << " getType: " << an_addr->getType () << " getCentraldId (): " << an_addr->getCentraldId () << " getHost (): " << an_addr->getHost () << " getHostNum (): " << an_addr->getHostNum () << " getCentraldNum (): " << an_addr->getCentraldNum () << std::endl;
	#endif
	addAddress (an_addr);
}
...
int Block::addAddress (NetworkAddress * in_addr)
{
	...
	#ifdef DEBUG_EXTRA
	std::cout << "addAddress2: getOpenConnection: " << conn << std::endl;
	#endif
	if (conn)
	{
		#ifdef DEBUG_EXTRA
		std::cout << "addAddress2: volame addressUpdated: " << conn << std::endl;
		#endif
	...
	else if (willConnect (in_addr))
	{
		...
		#ifdef DEBUG_EXTRA
		std::cout << "addAddress2: if willconnect, createClientConnection: " << conn << std::endl;
		#endif
		if (conn)
			addConnection (conn);
	}
...
{
	// base note: the classic tree switches on other_device_type to build
	// one of 15 typed DevClient subclasses (DevClientCamera,
	// DevClientTelescope, ...) - all deferred, see devclient.h. Every
	// device type gets the generic base DevClient here until a real
	// consumer needs typed behavior.
