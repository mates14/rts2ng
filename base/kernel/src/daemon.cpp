...
// base note: NOT yet wired into kernel/CMakeLists.txt - port continues with
// multidev next (task 7), then centrald (task 9). Dropped the classic
// tree's LOCK_SH/LOCK_EX/LOCK_NB/LOCK_UN fallback #defines and the
// RTS2_HAVE_FLOCK / lockf() fallback in checkLockFile/init - both were for
// platforms without flock(), which modern Linux always has.
...
{
	// base note (2026-07-16): every daemon logs locally via syslog under
	// one shared ident ("rts2") regardless of whether centrald is
	// reachable - see the rewritten Daemon::sendMessage below for why.
	openlog ("rts2", LOG_PID, LOG_DAEMON);
...
int Daemon::doDaemonize ()
{
	...
	// openlog("rts2", ...) for every daemon that actually daemonizes (the
	// normal case; only -i/interactive runs skip doDaemonize entirely),
	// defeating the whole point of one shared, grep/journalctl-able "rts2"
...
{
	// base note (2026-07-16): classic/faithfully-ported behavior only
	// syslog()'d when no centrald was reachable; otherwise a subclass
	// (Device) would relay the message over its centrald connections
	// instead, and centrald was the sole writer of one shared log file.
	// Under load that let routine logging traffic contend with the same
	// wire connections used for real device commands (see the
	// rts2_logging_architecture_flaw memory). Now every daemon always
	// logs locally via syslog (ident "rts2", set up once in the
	// constructor) - persistence no longer depends on centrald being
	// reachable at all. The wire relay to centrald (still done by
	// Device::sendMessage/Centrald::processMessage) is now purely for
	// live rts2-mon display, not persistence.
	int prio;
	...
	if (daemonize == DO_DAEMONIZE || daemonize == DONT_DAEMONIZE)
	{
		// not (yet) daemonized/interactive (-i) - also print to stderr so the user sees it directly
...
int Daemon::setValue (Connection * conn)
{
	...
	CondValue *old_value_cond = nullptr;
...
int Daemon::autosaveValues ()
{
	...
	std::ofstream of (autosaveFile.c_str(), std::ios_base::out | std::ios_base::trunc);
