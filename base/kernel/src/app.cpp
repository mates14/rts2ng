...
// base note: App::useLocalTime and rts2format's setLocalTimeDefault() are
// kept explicitly in sync here (see the "base note" in rts2format.h) -
// this replaces the classic tree's formatLocalTime() reaching up into
// getMasterApp()->usesLocalTime() directly.
...
int App::initOptions ()
{
	...
	long_option = new struct option[options.size () + 1];
	...
	*end_opt = '\0';
	...
	an_option->name = nullptr;
	an_option->has_arg = 0;
	an_option->flag = nullptr;
	an_option->val = 0;

	/* get attrs */
	while (1)
	{
		c = getopt_long (app_argc, app_argv, opt_char, long_option, nullptr);
...
static void signalHUP (int sig)
{
	if (masterApp)
		masterApp->sigHUP (sig);
}

static void killSignal (int sig)
{
	signal (SIGHUP, exit);
	signal (SIGINT, exit);
	signal (SIGTERM, exit);
...
This is free software, and you are welcome to redistribute it under certain conditions; see http://www.gnu.org for details."
				...
				<< std::endl
				<< std::endl
				<< "base - CMake port of the RTS2 backend"
				<< std::endl;
