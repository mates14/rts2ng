...
// base note: DevClientCamera/DevClientTelescope/DevClientFocus/
// DevClientPhot added for execcli/scriptexec (task 24) - see the note at
// the top of devclient.h for why the other 11 concrete subclasses remain
// deferred.
...
const char *DevClient::getName () { return connection->getName (); }

int DevClient::getOtherType () { return connection->getOtherType (); }

Block *DevClient::getMaster () { return connection->getMaster (); }

int DevClient::queCommand (Command * cmd) { return connection->queCommand (cmd); }

void DevClient::queCommand (Command * cmd, int notBop) { connection->queCommand (cmd, notBop); }
