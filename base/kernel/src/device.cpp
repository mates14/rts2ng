...
// base note: dropped the classic tree's #include <rts2-config.h> - grep
// confirms nothing in this file uses an RTS2_HAVE_* macro.
...
int DevConnection::init ()
{
	...
	if (ret)
	{
		logStream (MESSAGE_ERROR) << "NetworkAddress::getAddress getaddrinfo for host " << address->getHost () << ": " << (ret == EAI_SYSTEM ? strerror (saved_errno) : gai_strerror (ret)) << sendLog;
		return -1;
	}
...
Device::Device (int in_argc, char **in_argv, int in_device_type, const char *default_name):Daemon (in_argc, in_argv)
{
	...
	deviceStatusCommand = nullptr;

	doCheck = true;
	doAuth = true;

	last_weathermsg = nullptr;
	...
	addOption ('d', nullptr, 1, "name of device");
}
...
void Device::setWeatherState (bool good_weather, const char *msg)
{
	...
	if (good_weather == false && (getState () & BAD_WEATHER) && (last_weathermsg == nullptr || strcmp (msg, last_weathermsg)))
	{
