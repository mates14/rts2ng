...
int NUT::initHardware ()
{
	...
	// base note: classic tree only checked connNUT->init()'s return value,
	// but ConnTCP::init() throws ConnCreateError (rather than returning
	// nonzero) on most connect failures (e.g. connection refused) -
	// an uncaught exception here crashed the daemon. See UPSTREAM_BUGS.md.
	try
	{
		ret = connNUT->init ();
	}
	...
	{
		logStream (MESSAGE_ERROR) << "cannot connect to UPS " << upsName << " on host " << host->getHostname () << " port " << host->getPort () << ": " << er << sendLog;
		return -1;
	}
	if (ret)
	{
