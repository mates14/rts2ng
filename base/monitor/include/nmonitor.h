...
// base note: two classic dependencies deliberately dropped here (see
// base/STATUS.md "Monitor" section for the full writeup):
//  - "rts2fits/devcliimg.h" (DevClientCameraImage) - grep-verified dead
//    include in the classic tree; nothing in src/monitor/*.cpp ever
//    instantiates it. Dropping it avoids pulling in the whole
//    rts2fits/cfitsio image-writing subsystem for nothing.
//  - "simbadtarget.h" (SimbadTarget, used only by the optional
//    command-line target-name argument to auto-slew a mount on startup) -
//    requires libxml2 + RTS2's bundled xmlrpc++ client to query the
//    network SIMBAD service, a real but peripheral feature unrelated to
//    bus monitoring. tarArg, processArgs(), and the DEVICE_TYPE_MOUNT
//    special case in createOtherType() are dropped along with it.
#include <climits>
