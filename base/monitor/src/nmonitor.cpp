...
// base note: dropped the classic tree's RTS2_HAVE_XCURSES block
// (XCursesProgramName - PDCurses/X11 curses backend, irrelevant on modern
// Linux ncurses) and the __CYGWIN__ guard around ESCDELAY (always applies
// on Linux). RTS2_PACKAGE_VERSION -> BASE_VERSION. See nmonitor.h for the
// SimbadTarget/devcliimg deferral notes - processArgs()/tarArg and the
// DEVICE_TYPE_MOUNT special case in createOtherType() are dropped along
// with that.
...
void NMonitor::menuPerform (int code)
{
	switch (code)
	{
		...
		case MENU_ON:
			...
			break;
		case MENU_ABOUT:
			messageBox ("   rts2-mon " BASE_VERSION "\n"
