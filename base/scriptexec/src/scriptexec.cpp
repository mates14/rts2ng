 ...
 *
 * base note: classic ClientCameraScript also drove a live ncurses
 * progress-bar (exposure/readout percentage redrawn on a timer, terminal
 * width queried via curses). That's purely a terminal-UI cosmetic on top
 * of the same processImage() below and isn't needed to drive scripts
 * headlessly - dropped here, matching how other purely-cosmetic preview
 * paths were dropped elsewhere in this port (JPEG preview generation in
 * rts2image::Image). The important behavior - printing each written
 * image's filename to stdout, which the `xpaset`-piping examples in
 * usage() below depend on - is kept.
 *
