...
// base note: unlike SEP/GPointModel/TLE/etc elsewhere in this port, the
// Andor SDK is NOT deferred - a real (precompiled, vendor-shipped) copy of
// it is available on this system at /home/mates/src/andor (copied from the
// D50 install, not installed system-wide) and its x86_64 .so links cleanly
// (`ldd` resolves against only standard system libs). This target is
// therefore built for real, gated behind the BASE_ANDOR_SDK_DIR CMake
// cache variable (see camd/CMakeLists.txt) - empty by default, so the
// driver is skipped unless someone explicitly points at an SDK checkout,
// matching the classic tree's opt-in `--with-andor=<path>` and the fact
// that nobody has this hardware without knowing it. Build/link was
// verified on this system; actual hardware behavior (Initialize(), sensor
// readout, temperature control, etc.) was NOT and cannot be verified here
// - there is no physical camera attached.
//
// Modernization applied while porting: dropped the pre-GCC3 iostream.h/
// fstream.h fallback (`_GNUC3_` detection - dead on any modern compiler)
// and the unused `<fstream>` include (grep-verified: no ifstream/ofstream
// use anywhere in this file); `NULL` -> `nullptr`; bare `isnan` ->
// `std::isnan` (matching convention used elsewhere in base); dropped six
// declared-but-never-defined-and-never-called methods (`setADChannel`,
// `printInfo`, `printCapabilities`, `printNumberADCs`, `printHSSpeeds`,
// `updateHSSpeeds`, `printVSSpeeds`) - grep-verified dead in the classic
// tree too (`rts2-camd-andor` links from this single source file, so
// there's no other translation unit that could define them).
...
namespace rts2camd
{
  ...
  private:
    ...
    FILE *kinSeriesTarget = nullptr;
    rts2core::ValueString * kinFile;
    //char *kinFile = nullptr;
    uint64_t *kinSeriesBuffer = nullptr;
  ...
  if (kinSeriesTarget)
    {
      ...
      kinSeriesTarget = nullptr;
    }
  if (kinSeriesBuffer)
    {
      free (kinSeriesBuffer);
      kinSeriesBuffer = nullptr;
    }
  nullExposureConn ();
  AbortAcquisition ();
  FreeInternalMemory ();
...
Andor::doReadout ()
{
  ...
  // 15 seconds timeout for acqusition (increased from 10s to support low-noise digitization mode which takes 11.2s)
  if (getNow () > getExposureEnd () + 15)
    {
