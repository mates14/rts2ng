...
// base note: these event codes and structs are the wire between Camera's
// own filter/focuser control methods (setFilterNum, getFocPos, ...) and the
...
class Camera:public rts2core::ScriptDevice
{
	...
	protected:
		 ...
		 */
		void nullExposureConn () { exposureConn = nullptr; }
		...
		void createExpType ()
		{
			...
			expType->addSelVal ("LIGHT", nullptr);
			expType->addSelVal ("DARK", nullptr);
		}
		 ...
		 */
		void addShutterType (const char *enumName, rts2core::Rts2SelData *data = nullptr) { expType->addSelVal (enumName, data); }
		...
		void createTempSet ()
		{
			...
			addOption ('c', nullptr, 1, "night cooling temperature");
		}
		...
		virtual int box (int _x, int _y, int _width, int _height, rts2core::ValueRectangle *retv = nullptr);

		//* if >=0, camera custom drives data transfer; data isn't created on readout
		int realTimeDataTransferCount;

		void startExposureConnImageData () { startImageData (exposureConn); }

		/**
		 * Find stars using the SEP library, if available and enabled.
		 *
		 * base note: the classic implementation links against the vendored
		 * `sep` library (source-extractor-as-library) unconditionally. This
		 * is genuinely optional (sepFind defaults to false, it's a star-
		 * finding convenience feature, not needed for exposure/readout) and
		 * SEP is a vendor-tier dependency by the driver-tier model used
		 * throughout this port, so it's deferred here - stubbed to a no-op.
		 * Port properly (link against ../include/sep) if/when a driver
		 * needs it for real.
		 */
