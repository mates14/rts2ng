...
// base note: dropped #include "cliwheel.h"/"clifocuser.h" and #include
// "sep/sep.h". Filter-wheel/focuser DevClient subclasses
// (ClientFilterCamera/ClientFocusCamera) are deferred the same way as the
// rest of the DevClient family (see kernel/include/devclient.h) - dummy
// camd never configures a wheel/focuser device, so Camera::createOtherType
// falls through to the ScriptDevice/Device default for those device types
// instead of instantiating them. SEP star-finding is stubbed in
// findSepStars() - see the note on its declaration in camd.h.
...
Camera::Camera (int in_argc, char **in_argv, rounding_t binning_rounding):rts2core::ScriptDevice (in_argc, in_argv, DEVICE_TYPE_CCD, "C0")
{
	...
	dataChannels = nullptr;
	channels = nullptr;

	rts2ControlCooling = nullptr;

	tempAir = nullptr;
	tempCCD = nullptr;
	tempCCDHistory = nullptr;
	tempCCDHistoryInterval = nullptr;
	tempCCDHistorySize = nullptr;
	tempSet = nullptr;
	nightCoolTemp = nullptr;
	coolingOnOff = nullptr;
	...
	wcs_ctype1 = wcs_ctype2 = nullptr;
	wcs_crpix1 = wcs_crpix2 = nullptr;
	default_crpix[0] = default_crpix[1] = 0;

	wcs_cdelta1 = wcs_cdelta2 = wcs_crota = nullptr;
	...
	camFilterVal = nullptr;
	camFilterOffsets = nullptr;
	useFilterOffsets = nullptr;
	...
	addOption ('e', nullptr, 1, "default exposure");
...
{
	logStream (MESSAGE_DEBUG) << "****** killAll ()" << sendLog;
	timeReadoutStart = NAN;
...
int Camera::processOption (int in_opt)
{
	...
	switch (in_opt)
	{
		...
		case OPT_WHEEL:
			if (!strcmp (optarg, "-"))
				wheelDevices.push_back (nullptr);
			else
...
{
	if (wcs_cdelta1 == nullptr || wcs_cdelta2 == nullptr)
		return;
...
int Camera::initValues ()
{
	...
	if (sharedMemNum >= 0)
	{
		...
		if (sharedData->create (sharedMemNum, dataBufferSize) == nullptr)
		{
			return -1;
		}
	...
	focusingHeader = nullptr;

	if (tempCCDHistory != nullptr)
	{
	...
	if (wheelDevices.size () > 0 && conn->getOtherDevClient ())
	{
		...
		for (fiter = camFilterVals.begin (), iter = wheelDevices.begin (); fiter != camFilterVals.end () && iter != wheelDevices.end (); fiter++, iter++)
		{
			if (!strcmp (conn->getName (), *iter))
			{
				...
				if (val != nullptr && val->getValueType () == RTS2_VALUE_SELECTION)
				{
...
{
	// base note: SEP support deferred - see the declaration in camd.h.
	return;
}
...
char* Camera::getDataBuffer (int chan)
{
	...
	if (dataBuffers[chan] == nullptr && suggestBufferSize () > 0)
