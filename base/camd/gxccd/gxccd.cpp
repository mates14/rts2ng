...
// base note: like Andor, the Moravian Instruments (gxccd) SDK is NOT
// deferred - a real copy (headers, static .a and shared .so, both for
// x86_64) is available on this system at /home/mates/libgxccd-0.9.0.
// Gated behind the BASE_GXCCD_SDK_DIR CMake cache variable (see
// camd/CMakeLists.txt), empty/skipped by default - same opt-in reasoning
// as Andor's BASE_ANDOR_SDK_DIR (classic tree: --with-gxccd=<path>).
// The classic Makefile.am links the static libgxccd.a (+ -lrt, folded
// into glibc on this modern system so not listed separately); CMake's
// find_library() here resolves to the SDK's libgxccd.so instead (both
// ship in the SDK tree) - unlike Andor's .so, this one's SONAME
// ("libgxccd.so") matches its own filename exactly, so it needs only an
// RPATH into the SDK's lib/ dir, no symlink workaround. Also needs
// libusb-1.0 (has a working pkg-config file on this system, unlike
// libnova).
//
// GXCCD_HAS_RESET_FILTERS (guarding the "reset_filters" command below) is
// not defined anywhere in the classic tree's configure.ac/Makefile.am
// either, and this SDK version's gxccd.h doesn't declare
// gxccd_reset_filters() - so that branch is dead in both trees. Kept
// as-is (like andor.cpp's TEMP_SAFETY) in case a build ever defines it
// against a newer SDK version that has the function.
...
class GXCCD:public Camera
{
	...
	private:
		...
		rts2core::ValueIntegerMinMax *fan = nullptr;
		rts2core::ValueIntegerMinMax *windowHeating = nullptr;
		rts2core::ValueFloat *tempRamp;
		rts2core::ValueFloat *tempTarget;
		rts2core::ValueFloat *power = nullptr;
...
GXCCD::GXCCD (int argc, char **argv):Camera (argc, argv)
{
	...
	addOption ('w', nullptr, 1, "desired level of CCD window heating (autostarted with cooling, for night observations)");
	...
	addOption ('p', nullptr, 1, "MI CCD product ID");
	addOption ('f', nullptr, 1, "filter names (separated with :)");
