...
// base note: dropped the classic tree's optional direct-FITS-write readout
// path (#include "rts2fits/image.h", the fitsTransfer value and the
// image != NULL branches in doReadout()) - that path pulls in the whole
// rts2fits/cfitsio image-writing subsystem, which is a separate frontend-
// adjacent library, not part of the backend/kernel this port is building.
// fitsTransfer defaulted to false in the classic tree anyway; the real
// end-to-end path being proven here is TCP/IP readout over the wire
// protocol, which is fully preserved below.
...
void Dummy::generateImage (size_t pixelsize, int chan)
{
	...
	if (genType->getValueInteger () == 6)
	{
		...
		srandom (0 + time (nullptr) + getExposureNumber ());
	}
	...
	switch (getDataType ())
	{
		...
		case RTS2_DATA_LONGLONG:
			generateData ((int64_t *) getDataBuffer (chan), pixelsize);
			break;
