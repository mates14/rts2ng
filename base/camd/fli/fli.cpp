...
// base note: FLI camera driver, source of truth ../src/camd/fli.cpp.
// Same real SDK as the already-ported base/focusd/fli and
// base/filterd/fli (see base/STATUS.md "FLI" section for the full
// story of the two libfli copies found on this system - the one
// actually used, gated behind BASE_FLI_SDK_DIR, is
// /home/mates/src/fliusb/libfli, using FLI's own fliusb kernel module
// for USB I/O; it has a real FLIGetSerialString() and a const-correct
// FLIOpen(), so no workarounds were needed here either). Ported
// completely faithfully, NULL -> nullptr only.
...
Fli::Fli (int in_argc, char **in_argv):Camera (in_argc, in_argv)
{
	...
	addOption ('B', nullptr, 1, "FLI debug level (1, 2 or 3; 3 will print most error message to stdout)");
	addOption ('n', nullptr, 1, "Camera number (in FLI list)");
	addOption ('N', nullptr, 1, "Camera serial number (string, device name)");
	addOption ('b', nullptr, 0, "use background flush, supported by several newer CCDs (do not confuse with normal flush, initiated by parameter -l)");
	addOption ('l', nullptr, 1, "Number of CCD flushes");
	addOption ('f', nullptr, 1, "FLI device path");
...
int Fli::initHardware ()
{
	...
	if (fliDebug)
		FLISetDebugLevel (nullptr, fliDebug);
