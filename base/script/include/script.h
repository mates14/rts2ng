...
// base note: classic tree includes "rts2db/camlist.h" here purely for
// the free function getMaximalScriptDuration()'s signature
// (rts2db::CamList& parameter) - that function is called only from
// httpd/bbapi.cpp, printtarget.cpp and executorque.cpp, none of which are
// in the scriptexec closure, so both the function and the camlist.h
// include are dropped here (the one avoidable DB touch identified while
// scoping this port - see STATUS.md's "Scriptexec push" section).
