...
// base note: classic tree conditionally includes elementacquire.h and
// compiles the ACQUIRE ("A") script command behind
// `#ifdef RTS2_HAVE_PGSQL` - base never defines that macro (no database
// layer, see STATUS.md's "Scriptexec push" section), so that branch is
// simply never compiled here, same as in a classic build configured
// without Postgres support. Nothing else in this file changed.
