...
void DevScript::postEvent (rts2core::Event * event)
{
	...
	switch (event->getType ())
	{
		case EVENT_STOP_TARGET:
			logStream(MESSAGE_DEBUG) << script_connection->getName () << " EVENT_STOP_TARGET" << sendLog;
			if (currentTarget)
			{
		...
		case EVENT_SET_TARGET_KILL:
			logStream(MESSAGE_DEBUG) << script_connection->getName () << " EVENT_SET_TARGET_KILL" << sendLog;
		case EVENT_SET_TARGET_KILL_NOT_CLEAR:
			logStream(MESSAGE_DEBUG) << script_connection->getName () << " EVENT_SET_TARGET_KILL_NOT_CLEAR" << sendLog;
			currentTarget = NULL;
			nextTarget = NULL;
		#ifdef DEBUG_EXTRA
			logStream(MESSAGE_DEBUG) << script_connection->getName () << " EVENT_KILL_ALL" << sendLog;
			...
			switch (event->getType ())
			{
				case EVENT_STOP_TARGET:
				case EVENT_SET_TARGET_KILL:
					logStream(MESSAGE_DEBUG) << script_connection->getName () << " EVENT_SET_TARGET_KILL/EVENT_STOP_TARGET podruhe!" << sendLog;
					killTarget = (Rts2Target *) event->getArg ();
				case EVENT_KILL_ALL:
					logStream(MESSAGE_DEBUG) << script_connection->getName () << " EVENT_KILL_ALL podruhe!" << sendLog;
