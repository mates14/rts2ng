...
{
	// base note: classic tree ran an optional "after_exposure_cmd" here via
	// rts2plan::ConnImgProcess before queueing the image processor - that
	// class needs rts2db::Observation (real DB dependency), so this feature
	// is dropped here; base always queues straight to the lowest-queue
	// image processor connection below.
	(void) run_after;
...
void DevClientTelescopeExec::postEvent (rts2core::Event * event)
{
	...
	switch (event->getType ())
	{
		...
		case EVENT_SET_TARGET:
		case EVENT_SET_TARGET_NOT_CLEAR:
			currentTarget = (Rts2Target *) event->getArg ();
			break;
		case EVENT_SET_TARGET_KILL:
		case EVENT_SET_TARGET_KILL_NOT_CLEAR:
			clearWait ();
		...
		case EVENT_SLEW_TO_TARGET_NOW:
			if (currentTarget)
			{
				...
				ret = syncTarget (event->getType () == EVENT_SLEW_TO_TARGET_NOW, ((rts2core::ValueInteger *) event->getArg ())->getValueInteger ());
				switch (ret)
				{
					case OBS_DONT_MOVE:
