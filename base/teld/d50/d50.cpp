...
// base note: the D50 mount actually in use at Ondrejov - see STATUS.md's
// "Teld-d50" section for the full dependency chain ported to support this
// (ConnSerial/ConnEthernet in the kernel, TGDrive/ConnREMOTES/Fork in
// base/teld/). Modernization: NULL->nullptr throughout; dropped the
// `#ifndef RTS2_LIBERFA`/`#endif` guard pairs, keeping their body
// unconditional (matches teld.h/teld.cpp's ERFA removal - RTS2_LIBERFA is
// never defined in base, so these were always-true branches anyway).
// DEBUG_EXTRA/DEBUG_BRUTAL remain as opt-in developer logging toggles,
// same as classic (both `#define`s commented out by default).

//#define DEBUG_EXTRA
//#define DEBUG_BRUTAL

#include "tgdrive.h"
#include "connremotes.h"
#include "fork.h"
...
class D50:public Fork
{
	...
	private:
		...
		* @param *speedAtInspectionTime		where the computed speed at inspectionTime is stored (only when pointer differs from nullptr)
		*
		* @return computed position [counts] at inspectionTime, or time [ms] needed to complete all movemens, if inspectionTime<0
		*/
		int32_t computeMovement (int isDec, int32_t fromCount, int32_t toCount, double initSpeed, double inspectionTime, double* speedAtInspectionTime = nullptr);
...
int D50::init ()
{
	...
	// base note: classic tree did not check this return value - see
	// ConnREMOTES::init()'s base note. A failed raw-socket open here
	// used to leave the connection's buffers uninitialized, causing a
	// segfault on the very next register read/write below.
	ret = remotesRA->init ();
	if (ret)
		return ret;
...
int32_t D50::computeMovement (int isDec, int32_t fromCount, int32_t toCount, double v0, double inspectionTime, double* speedAtInspectionTime)
{
	...
	if (S_const >= 0)	// i.e., we reach the full speed during the transition
	{
		...
		{
			if (speedAtInspectionTime != nullptr)
...
{
	rts2core::Connection *connDome = nullptr;
	rts2core::Value *tmpValue = nullptr;
	...
	if (tmpValue == nullptr)
	{
		logStream (MESSAGE_ERROR) << "getDomeState: infotime is nullptr..." << sendLog;
		return -1;
	}
