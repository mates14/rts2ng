...
int Gemini::startResync ()
{
	...
	willFlip = false;
	// base note: classic tree left these uninitialized; if neither flip in
	// the loop below is possible, the debug log a few lines down reads them
	// unset. Zero-initializing avoids that UB without changing behavior in
	// the normal case where at least one flip is possible.
