...
// base note: three classic subsystems are deliberately deferred here -
// see STATUS.md's "Teld" section for the full writeup, and the matching
// notes in teld.cpp next to each stubbed method:
//  - TLE/satellite propagation (pluto SGP4/SDP4/SGP8/SDP8) - tle_t stays a
//    real member (pluto/norad.h is ported for the type), but
//    calculateTLE()/moveTLE()/parseTLE() are stubbed to log and fail
//    instead of calling into the ~3100-line NORAD propagation library.
//  - Concrete pointing models (GPointModel/TPointModel) - the abstract
//    TelModel interface is ported for real, but the two concrete
//    implementations are not; model stays nullptr unless a driver
//    explicitly loads a model file (dummy never does).
//  - Cupola/rotator DevClient subclasses (ClientCupola/ClientRotator) -
//    deferred like camd's ClientFilterCamera; createOtherType() falls
//    through to the base Device implementation for those device types.
//
// Separately (not a deferral, a build-target decision): the classic header
// carries the whole class in two flavors selected by #ifdef RTS2_LIBERFA -
// an ERFA/SOFA-based path and a manual libnova-based fallback. liberfa is
// not installed on this system and base's CMake does not probe for it, so
// only the non-ERFA (manual aberration/nutation/precession/refraction)
// path is ported - it is a complete, correct implementation on its own,
// not a stub. Add ERFA detection to camd's/kernel's CMakeLists and restore
// the #ifdef if a build with it is ever needed.
...
class Telescope:public rts2core::Device
{
	public:
		...
		void setModel (rts2telmodel::TelModel *_model) { model = _model; calModel->setValueBool (model != nullptr); }
	...
	private:
		...
		// base note: dropped `useErfa` (ValueBool*) - grep-verified dead in
		// the classic tree: declared but never createValue()'d or read
		// anywhere in teld.cpp.
