...
// base note: classic tree has this at include/connection/remotes.h -
// flattened here, placed in base/teld/ rather than the kernel since it's
// D50-specific (REMOTES units are custom Ethernet I/O boards used only by
// D50's mount), even though it's namespaced rts2core like the rest of the
// classic connection/ family.

#pragma once

#include "connethernet.h"
...
class ConnREMOTES:public ConnEthernet
{
	public:
		 ...
		 * @param writeBuf        buffer where data to be sent are stored; when nullptr, do only read operation
