...
// base note: source of truth classic include/tpointmodelterm.h +
// lib/rts2tel/tpointmodelterm.cpp. Faithful port - these are precision
// astrometric formulas (T-Point term definitions), not touched beyond
// include-path flattening and NULL->nullptr. Needed so GeminiUDP (and any
// future driver) can actually load a --t-point-model file - previously
// deferred (see telmodel.h's base note) since no driver needed it; now
// needed for real, a live T-Point model file (t0-191230.mod) is in use.
...
class TermHarmonics:public TPointModelTerm
{
	...
	private:
		...
		// named getHarmonicValue (not getValue) - avoids hiding
		// rts2core::ValueDouble::getValue(), a real -Woverloaded-virtual
		// this port surfaced (harmless here: this is a private helper,
		// never called polymorphically as a Value), but worth naming
		// around rather than leaving as recurring build noise.
		double getHarmonicValue (struct ln_equ_posn *pos, double tel_latitude_r, int i);
