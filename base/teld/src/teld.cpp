...
// base note: dropped #include "clicupola.h"/"clirotator.h" (ClientCupola/
// ClientRotator, deferred DevClient family members - see teld.h) and
// "gpointmodel.h"/"tpointmodel.h" (deferred concrete pointing models - see
// telmodel.h). Kept "pluto/norad.h" (tle_t type only) and "pluto/observe.h"
// (ported for real, self-contained geometry helpers). Kept "dut1.h"
// (getDUT1 real, retrieveDUT1 stubbed - see dut1.cpp).
...
Telescope::Telescope (int in_argc, char **in_argv, bool diffTrack, bool hasTracking, int hasUnTelCoordinates, bool hasAltAzDiff, bool parkingBlock, bool hasDerotators):rts2core::Device (in_argc, in_argv, DEVICE_TYPE_MOUNT, "T0")
{
	...
	{
		diffTrackRaDec = nullptr;
		diffTrackStart = nullptr;
		diffTrackOn = nullptr;
	}
	...
	{
		diffTrackAltAz = nullptr;
		diffTrackAltAzStart = nullptr;
		diffTrackAltAzOn = nullptr;
	}

	if (hasTracking)
	{
		createValue (tracking, "TRACKING", "telescope's actual tracking type", true, RTS2_VALUE_WRITABLE);
		tracking->addSelVal ("off");
		tracking->addSelVal ("on");
		tracking->addSelVal ("sidereal");
		if (hasDerotators)
			tracking->addSelVal ("off derotators");		// normal tracking, excluding derotators (for service purposes etc)

		createValue (defaultTracking, "default_tracking", "telescope's default tracking type", false, RTS2_VALUE_WRITABLE);
		defaultTracking->addSelVal ("off");
		defaultTracking->addSelVal ("on");
		defaultTracking->addSelVal ("sidereal");
		if (hasDerotators)
			defaultTracking->addSelVal ("off derotators");
		defaultTracking->setValueInteger (1);
	...
	{
		tracking = nullptr;
		trackingInterval = nullptr;
		trackingFrequency = nullptr;
		trackingWarning = nullptr;
		skyVect = nullptr;
	}
	...
	tPointModelFile = nullptr;
	rts2ModelFile = nullptr;
	model = nullptr;
	...
	addOption ('l', nullptr, 1, "separation limit (corrections above that number in degrees will be ignored)");
	addOption ('g', nullptr, 1, "minimal good separation. Correction above that number will be aplied immediately. Default to 180 deg");

	addOption ('c', nullptr, 1, "minimal value for corrections. Corrections bellow that value will be rejected.");

	addOption ('s', nullptr, 1, "park when switched to standby - 1 only at day, 2 even at night");
	addOption (OPT_BLOCK_ON_STANDBY, "block-on-standby", 0, "block telescope movement when switching to standby");

	addOption ('r', nullptr, 1, "telescope rotang");
...
int Telescope::checkTracking (double maxDist)
{
	...
	if (targetDistanceStat != nullptr && (getState () & (TEL_MASK_MOVING | TEL_MASK_CORRECTING | TEL_MASK_OFFSETING)) == TEL_OBSERVING)
	{
		...
		else if (ret == -2)
		{
			...
			else
			{
				if ((woffsRaDec->wasChanged () || wcorrRaDec->wasChanged ()))
					...
					startResyncMove (nullptr, (woffsRaDec->wasChanged () ? 1 : 0) | (wcorrRaDec->wasChanged () ? 2 : 0));
				else
...
{
	// base note: ClientCupola/ClientRotator (DevClient subclasses) are
	// deferred - see teld.h. Fall through to the base implementation for
	// both device types, same as Device::createOtherType() does for every
	// device type; the cupola/rotator connection still gets tracked in
	// cupolas/rotators and still receives CommandCupolaSyncTel via
	// queueCommandForType(), it just won't get the specialized sync-status
	// event callbacks ClientCupola/ClientRotator would have provided.
...
void Telescope::changeMasterState (rts2_status_t old_state, rts2_status_t new_state)
{
	...
	if ((getState () & STOP_MASK) != STOP_EVERYTHING && (new_state & STOP_MASK) == STOP_EVERYTHING)
	{
		...
		stopTracking ("tracking blocked");
		stopMove ();	// must be here explicitly (but it's already in stopTracking () for most cases)
	...
	if (dut1fn != nullptr && (new_state & SERVERD_STATUS_MASK) == SERVERD_EVENING)
	{
		requestDUT1();
	}
...
{
	// base note: GPointModel is deferred - see telmodel.h.
	logStream (MESSAGE_ERROR) << "reloading pointing model from stream is not supported in base - GPointModel was not ported (see STATUS.md)" << sendLog;
...
int Telescope::setTracking (int track, bool addTrackingTimer, bool send, const char *stopMsg)
{
	if (tracking != nullptr)
	{
		...
		else
		{
			...
			stopMove ();
			if (stopMsg != nullptr)
				maskState (TEL_MASK_TRACK, TEL_NOTRACK, stopMsg);
		}
		if (send == true)
...
int Telescope::startTracking (bool check)
{
	if (tracking == nullptr)
		return 0;
	...
	{
		tracking->setValueInteger (defaultTracking->getValueInteger ());
	}
	trackingFrequency->clearStat ();
	return setTracking (tracking->getValueInteger (), true);
}

int Telescope::stopTracking (const char *stopMsg)
{
	return setTracking (0, false, true, stopMsg);
}
...
{
	// base note: NORAD SGP4/SDP4 propagation is deferred - see teld.h and
	// pluto/norad.h. The classic implementation calls parse_elements()
	// here; ported as a hard failure instead.
	logStream (MESSAGE_ERROR) << "TLE (satellite) tracking is not supported in base - NORAD SGP4/SDP4 propagation was not ported (see STATUS.md)" << sendLog;
	return DEVDEM_E_PARAMSVAL;
}

int Telescope::parseTLE (rts2core::Connection *conn, const char *l1, const char *l2)
{
	return moveTLE (l1, l2);
}
...
{
	// base note: NORAD SGP4/SDP4 propagation is deferred - see teld.h and
	// pluto/norad.h. moveTLE()/parseTLE() already reject any actual TLE
	// from being set (tle_l1/tle_l2 stay empty), so this should never be
	// reached in practice - kept as a safe no-op rather than an assert.
	logStream (MESSAGE_ERROR) << "TLE (satellite) tracking is not supported in base - calculateTLE() called unexpectedly" << sendLog;
	ra = NAN;
	dec = NAN;
	dist_to_satellite = NAN;
}
...
void Telescope::startCupolaSync ()
{
	...
	queueCommandForType (DEVICE_TYPE_CUPOLA, cmd, nullptr, true);
	nextCupSync = n + 2;
}
...
int Telescope::startResyncMove (rts2core::Connection * conn, int correction, bool _ignoreHorizon)
{
	...
	else
	{
		...
		logStream (INFO_MOUNT_SLEW_START | MESSAGE_INFO) << telRaDec->getRa () << " " << telRaDec->getDec () << " " << pos.ra << " " << pos.dec << " " << hrz.alt << " " << hrz.az << " " << hrztar.alt << " " << hrztar.az << sendLog;
...
{
	// base note: GPointModel/TPointModel reload is deferred - see
	// telmodel.h. Report an error instead of silently ignoring a HUP-
	// triggered reload request if a model file was actually configured.
	if (tPointModelFile)
	{
		logStream (MESSAGE_ERROR) << "cannot reload T-Point model - TPointModel was not ported in base (see STATUS.md)" << sendLog;
	}
	if (rts2ModelFile)
	{
		logStream (MESSAGE_ERROR) << "cannot reload RTS2 model - GPointModel was not ported in base (see STATUS.md)" << sendLog;
	}
...
int Telescope::moveAltAz (bool _ignoreHorizon)
{
	...
	return startResyncMove (nullptr, 0, _ignoreHorizon);
}
...
int Telescope::commandAuthorized (rts2core::Connection * conn)
{
	...
	else if (conn->isCommand ("worm_stop"))
	{
		if (!conn->paramEnd ())
			return DEVDEM_E_PARAMSNUM;
		stopTracking ("tracking stopped by worm_stop command");
		return 0;
	}
