...
int ConnREMOTES::init ()
{
	...
	// base note: classic tree discarded ConnEthernet::init()'s return
	// value here and always returned 0. If the raw socket fails to open
	// (e.g. missing CAP_NET_RAW, or any other reason), ConnEthernet's
	// buffer pointers are never allocated, and the first read/write call
	// on this connection dereferences them uninitialized -> segfault.
	// Found while smoke-testing rts2-teld-d50 without root privileges.
...
{
	return write (registerCode, nullptr, 0, readBuf, readLen);
}
...
int ConnREMOTES::write (unsigned short registerCode, unsigned char *writeBuf, int writeLen, unsigned char *readBuf, int readLen)
{
	...
	if (writeBuf==nullptr)	// only "read" operation
	{
