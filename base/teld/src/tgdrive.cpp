...
void TGDrive::ecRead (char *msg, int len)
{
	...
	do
	{
		if (escaped > 0)
		{
			...
			// if the string ends with a single MSG_START char, there surely should be one more, let's read it
		...
		for (; checked_end < clen; checked_end++)
		{
			...
			{
				// if we are in the end of read data and have only one MSG_START, there must be the second one in the buffer, let's read it (and we will continue from the same position in the next run)
