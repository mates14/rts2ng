...
{
	// base note: non-ERFA branch only (see teld.h) - manual libnova UTC
	// julian date instead of getEraUTC() (ERFA-based).
	double utc1, utc2;
...
int Dummy::sky2counts (const double utc1, const double utc2, struct ln_equ_posn *pos, struct ln_hrz_posn *hrz_out, int32_t &ac, int32_t &dc, bool writeValues, double haMargin, bool forceShortest)
{
	ac = pos->ra * 10000;
	dc = pos->dec * 10000;
	applyCorrections (pos, utc1, utc2, nullptr, writeValues);
	if (writeValues)
		setTelTarget (pos->ra, pos->dec);
	return 0;
}
