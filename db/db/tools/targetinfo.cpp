...
// base/db note: classic TargetInfo also had a --auger-id option that
// instantiated rts2db::TargetAuger directly. TargetAuger is not ported
// (see db/STATUS.md "deliberately deferred" - no Auger-network access
// to test against), so that option/branch is dropped here rather than
// faithfully ported into a broken state.
