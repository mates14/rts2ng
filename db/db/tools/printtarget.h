...
// base/db note: classic printtarget.h/.cpp lived in lib/rts2script/ as a
// reusable library piece (several classic src/db and src/plan tools linked
// it). In this port only rts2-targetinfo uses it, so it's kept local to
// db/db/tools/ rather than promoted into a shared library - see
// db/STATUS.md.
...
// base/db note: classic rts2script::getMaximalScriptDuration() (needs
// rts2db::CamList) was deliberately dropped when base/script was ported -
// its only callers at the time (httpd/bbapi.cpp, executorque.cpp, and this
// file) were either unported or, for executorque.cpp, confirmed dead code
// (see db/executor's executorque.h comment). printtarget.cpp is a real,
// live caller now, so it's revived here rather than in base/script, to
// keep base/script itself DB-free.
