...
void Rts2NewTarget::usage ()
{
	std::cout << "You can specify target on command line. Arguments must be in following order:" << std::endl
		...
		<< std::endl
		<< "To enter new target called NGC567, with ID 1003:" << std::endl
...
int Rts2NewTarget::processArgs (const char *arg)
{
	if (n_tar_id == -1)
		n_tar_id = atoi (arg);
	else if (n_tar_name == nullptr)
		n_tar_name = arg;
	else if (n_tar_ra_dec == nullptr)
		n_tar_ra_dec = arg;
	else
		return -1;
	return 0;
...
int Rts2NewTarget::doProcessing ()
{
	...
	int ret;
	// ask for target name..
	if (n_tar_ra_dec == nullptr)
	{
		if (n_tar_name == nullptr)
		{
			std::cout << "Default values are written inside [].." << std::endl;
			ret = askForObject ("Target name, RA&DEC or MPC one line ephemeris");
		}
		else
		{
			ret = askForObject ("Target name, RA&DEC or MPC one line ephemeris", std::string (n_tar_name));
		}
	}
	else
	{
		ret = askForObject ("Target, RA&DEC or MPC one line ephemeris", std::string (n_tar_ra_dec), *n_tar_ra_dec ? false : true);
	}
	if (ret)
		return ret;
