...
Target *createTargetByString (std::string tar_string, bool debug)
{
	...
	// base note: classic createTargetByString() fell through to TLE parsing, then
	// SIMBAD name resolution, then an MPEC-catalog fallback here. None of those are
	// ported yet (see targetres.h) - fail loudly instead of guessing.
	throw rts2core::Error ("cannot resolve '" + tar_string + "' as RA DEC or MPC ephemeris "
		"(SIMBAD/MPEC/TLE target resolution is not ported yet)");
}
