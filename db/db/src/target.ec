...
// db note: classic file also included "rts2db/target_auger.h" and
// "rts2db/tletarget.h" for the TYPE_AUGER/TYPE_TLE createTarget() cases
// below. Neither TargetAuger (needs Auger-network-specific DB schema the
// user has no access to test) nor TLETarget (needs the ~3200-line SGP4/SDP4
// satellite propagator, only lib/pluto/observe.cpp of which made it into
// base so far - see teld-d50's deferred TLE/SGP4 note) is ported yet.
// Those two createTarget() cases below throw instead of instantiating -
// see the comments at each case.
#include "rts2db/targetell.h"
#include "rts2db/rts2targetplanet.h"
...
void Target::loadTarget (int in_tar_id)
{
	...
	if (d_tar_bonus_time_ind >= 0)
		tar_bonus_time = (time_t) d_tar_bonus_time;
	else
		tar_bonus_time = 0;

	if (d_tar_next_observable_ind >= 0)
		tar_next_observable = (time_t) d_tar_next_observable;
	else
		tar_next_observable = 0;
...
{
	return labels.getTargetLabels (getTargetID (), LABEL_PROGRAM).getString ("", ",");
}
...
int Target::selectedAsGood ()
{
	...
	if (getTargetEnabled () && tar_priority + tar_bonus >= 0)
	{
		if (d_tar_next_observable_ind >= 0)
		{
			time_t now;
			time (&now);
			if (now < (time_t) d_tar_next_observable)
				return -1;
		}
		return 0;
	}
...
bool Target::isAboveHorizon (double JD)
{
	struct ln_hrz_posn hrz;
	getAltAz (&hrz, JD);
...
Target *createTarget (int _tar_id, struct ln_lnlat_posn *_obs, double _altitude)
{
	...
	switch (db_type_id)
	{
		...
		case TYPE_ELLIPTICAL:
			...
			break;
		case TYPE_TLE:
			// db note: TLETarget not ported yet (needs the SGP4/SDP4
			// propagator - see the file-level comment above). Fail loudly
			// rather than silently returning wrong-behaving ConstTarget.
			throw rts2core::Error ("TLE targets are not supported by this build of db yet (SGP4/SDP4 propagator not ported)");
		case TYPE_GRB:
		...
		case TYPE_TERESTIAL:
			...
			break;
		case TYPE_PLAN:
			// db note: the Plan/TargetPlan target-chaining feature was
			// removed as part of the executor-lite pass (see
			// rts2_executor_lite_plan memory) - it was superseded by, and
			// "desperately failed" in practice compared to, the Python
			// queuer. Fail loudly rather than silently mishandling it.
			throw rts2core::Error ("Plan targets are no longer supported - the Plan feature was removed in favour of the Python queuer");
		case TYPE_AUGER:
			// db note: TargetAuger not ported yet (Auger-network-specific
			// schema/fields, no test access - see the file-level comment
			// above). Fail loudly rather than silently returning
			// wrong-behaving ConstTarget.
			throw rts2core::Error ("Auger targets are not supported by this build of db yet (TargetAuger not ported)");
		case TYPE_PLANET:
...
void Target::sendInfo (Rts2InfoValStream & _os, double JD, int extended)
{
	...
	_os << (isAboveHorizon (JD) ? "Target is above local horizon." : "Target is below local horizon, it's not possible to observe it.") << std::endl;
	printExtra (_os, JD);
}
