...
moveType DarkTarget::startSlew (struct ln_equ_posn *position, std::string &p1, std::string &p2, bool update_position, int plan_id)
{
...
void CalibrationTarget::load ()
{
	...
	SELECT
		air_airmass_start,
		air_airmass_end,
		CAST (EXTRACT (EPOCH FROM air_last_image) AS INTEGER)
	FROM
		airmass_cal_images
	ORDER BY
		air_last_image asc,
...
void TargetSwiftFOV::load ()
{
	...
	SELECT
		...
		swift_dec,
		swift_roll,
		CAST (EXTRACT (EPOCH FROM swift_time) AS INTEGER),
		swift_obstime,
		swift_name
	FROM
		swift
...
void TargetIntegralFOV::load ()
{
	...
	SELECT
		...
		integral_ra,
		integral_dec,
		CAST (EXTRACT (EPOCH FROM integral_time) AS INTEGER)
	FROM
		integral
	WHERE
...
// db note: classic tree had a TargetPlan implementation here (Plan-based
// target chaining). Removed as part of the executor-lite pass - see the
// comment in rts2db/target.h and the rts2_executor_lite_plan memory.
