...
void ObservationSetDate::load (int year, int month, int day, int hour, int minutes)
{
	...
	while (1)
	{
		...
		// base note: classic checks `d_value2 == d_value2g` here - but
		// d_value2 was just reset to -1 a few lines above in the
		// `d_value1 == d_value2` branch below (this comparison runs before
		// that reset on this iteration, so it's not yet -1 here; still,
		// comparing d_value2 to d_value2g when the intent is clearly to
		// check whether d_value1's row matched d_value2g looks like a
		// copy-paste bug - see UPSTREAM_BUGS.md). Ported faithfully,
		// unchanged, since this path (ObservationSetDate, date-grouped
		// browsing) isn't on rts2-executor's critical path.
