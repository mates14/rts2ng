...
{
	// base note: classic code had this inverted - it set observer = _obs
	// (i.e. NULL) exactly when _obs was NULL, and ignored a real _obs when
	// one was given. Since the default constructor argument is NULL, the
	// common no-arg call (e.g. std::sort(..., sortByAltitude())) always
	// left observer == NULL, which doSort() then dereferences via
	// getAltAz(..., observer) - a near-guaranteed crash. See
	// UPSTREAM_BUGS.md.
	if (_obs)
		observer = _obs;
	else
...
{
	// base note: same inverted-condition bug as sortByAltitude above - see
	// UPSTREAM_BUGS.md.
	if (_obs)
		observer = _obs;
	else
