...
#include <math.h>
// db note: classic file also included "nan.h" - that header only defines
// a NAN fallback for WIN32/sun/pre-C99 platforms; modern glibc's <math.h>
// (above) already provides NAN unconditionally, so it's dropped (same
// "drop dead legacy-platform branches" reasoning used throughout base).
